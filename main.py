...
async def handle_feature(call: types.CallbackQuery):
    text = (
        "⚠️ You don't have access to this feature. Please authorise on the site: "
        "[Our Link]({}) to proceed.".format(AUTH_URL)
    )
    await call.message.answer(text, parse_mode="Markdown", reply_markup=get_main_keyboard())
    await call.answer("You must authorise first!", show_alert=True)
